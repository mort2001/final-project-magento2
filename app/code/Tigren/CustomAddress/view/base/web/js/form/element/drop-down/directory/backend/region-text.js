...
define([
    ...
    return Abstract.extend({
        ...
        initialize: function () {
            ...
            if (this.isBackend) {
                if (this.customScope !== 'billingAddress') {
                    this.value($(this.backendCreateOrderFields.shippingRegionInput).val());
                    if ($(this.backendCreateOrderFields.sameAsBillingInput).is(':checked'))
                        this.disabled(true);
                } else {
                    this.value($(this.backendCreateOrderFields.billingRegionInput).val());
                }
        ...
        onUpdate: function () {
            ...
            if (this.isBackend) {
                if (this.customScope === 'billingAddress') {
                    $(this.backendCreateOrderFields.billingRegionInput).val(this.value());
                    if ($(this.backendCreateOrderFields.sameAsBillingInput).is(':checked')) {
                        $(this.backendCreateOrderFields.shippingRegionInput).val(this.value());
                        $(this.uiBackendFields.uiShippingRegionInput).val(this.value());
                    }
