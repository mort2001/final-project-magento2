...
use Tigren\CustomAddress\Model\Import\City\RowValidatorInterface as ValidatorInterface;
use Tigren\CustomAddress\Model\Import\City\Validator;
...
{
    const ENTITY_CODE = 'custom_city';

    const TABLE_MAIN = 'directory_region_city';
    const TABLE_LOCALE = 'directory_region_city_name';

    const COL_REGION_CODE = 'region_code';
    ...
    public function validateRow(array $rowData, $rowNum)
    {
        ...
        $regionCode = $rowData[self::COL_REGION_CODE] ?? '';
    ...
    protected function saveEntityFinish(array $entityList)
    {
        ...
        foreach ($entityList as $entityData) {
            ...
            if ($cityId = $this->checkIfCityDataExists(
                ...
                $regionId = $this->convertRegionCodeIntoRegionId($mainData[self::COL_REGION_CODE]);
                $mainData = array_merge($mainData, ['region_id' => $regionId]);
                unset($mainData['region_code']);
     ...
     * @return mixed|null
     */
    protected function checkIfCityDataExists($regionCode, $code)
    {
        $regionId = $this->convertRegionCodeIntoRegionId($regionCode);
        if ($regionId) {
     ...
     * @return mixed
     */
    protected function checkIfLocaleDataExists($cityId, $locale)
    {
        $select = $this->connection->select()
     ...
     * @return mixed
     */
    protected function convertRegionCodeIntoRegionId($regionCode)
    {
        $regionCollection = $this->regionCollectionFactory->create();
        $regionCollection->addFieldToFilter('main_table.code', $regionCode);
        return $regionCollection->getFirstItem()->getRegionId();
    }
}
